...
class RedManager {
  ...
  async init(express) {
    ...
    express.post('/red/node/file', async function (req, res) {
          ...
          decompress(sampleFile.data, pathExtract).then((files) => {
            tar.pack(pathExtract, {
              ...
              .on('finish', async () => {
                ...
                await RED.runtime.nodes.addModule(moduleToAdd).then(() => {
                  return res.status(200).send('Module installed')
                }).catch((err) => { return res.status(400).send(err.message) })
              })
          })
        } catch (err) { return res.status(err.status).send(err.code) }
      }
      ...
      else return res.status(400).send('Wrong extension. Supported extension are : .zip, .tar and .tar.gz')
    })
  }
}
